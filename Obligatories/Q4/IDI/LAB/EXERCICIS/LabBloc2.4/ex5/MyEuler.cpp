  ...
  if(interaccio == ROTACIO){
    ...
    if (dx > dy){   // gir psi respecte eixY
      if (e->x() > xClick){
        ...
        setPsi((float) psi * 360.0 / (float)M_PI);
      ...
      if (e->y() > yClick){
        ...
        setTheta((float) theta * 360.0 / (float)M_PI);
    }
    update ();
  }
...
void MyEuler::createBuffers (std::string model_a_pintar)
{
  // Càrrega del model
  ...
  // Creació del Vertex Array Object per pintar el model
...
void MyEuler::pintaLegoman() {
    makeCurrent();
    createBuffers("legoman");
    inicialitzaCamera();
    update ();
}

void MyEuler::pintaPatricio() {
    makeCurrent();
    createBuffers("Patricio");
    inicialitzaCamera();
    update ();
