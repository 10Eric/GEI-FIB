...
{	vec3 NMNormal1;
  ...
  vec3 Phong;
    if (cam == 1 && esc == 1) {
     ...
     Phong = Especular(NMNormal1,L,vertSCO,colorFocus) + Especular(NMNormal1,L1,vertSCO,colorFocus);
    FragColor = vec4(Amb + Dif + Phong,1);
    }
    else if (cam == 1 && esc == 0) {
     Dif = Difus(NMNormal1,L1,colorFocus);
     Phong = Especular(NMNormal1,L1,vertSCO,colorFocus);
    FragColor = vec4(Amb + Dif + Phong,1);
    }
    else if (cam == 0 && esc == 1) {
     Dif = Difus(NMNormal1,L,colorFocus);
     Phong = Especular(NMNormal1,L,vertSCO,colorFocus);
    FragColor = vec4(Amb + Dif + Phong,1);
    }
	FragColor = vec4(Amb + Dif + Phong,1);	
}
