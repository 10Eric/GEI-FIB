...
public class New_Driver_Prestatgeria {
    private static New_Controlador_Prestatgeria controladorPrestatgeria;
    private static ControladorProducte controladorProducte;

    private static Scanner scanner = new Scanner(System.in);

    public static void main(String[] args) {
        crearDistribucionInicial();
        mostrarOpcionesPrestatgeria();
    }
    ...
    private static void crearDistribucionInicial() {
        ...
        controladorPrestatgeria = new New_Controlador_Prestatgeria();
        controladorPrestatgeria.setAltura(altura);
    }

    private static void mostrarOpcionesPrestatgeria() {
        while (true) {
            ...
            System.out.println("7. Consultar Distribucion");
            ...
            switch (opcion) {
                ...
                case 7 -> consultaDistribucio_Ordenada_Actual();  //imprime la prestatgeria entera
                case 8 -> calcular_distribucion(); //imprime al estanteria, calcula y luego la vuelve a imprimr
                case 9 -> {
    ...
    private static void calcular_distribucion() {
        if (controladorPrestatgeria.getProductesColocats().isEmpty()) {
            System.out.println("ERROR - No hay productos en la prestatgeria.");
            return;
        }
        ...
        List<ProducteColocat> arrangedProductes = controladorPrestatgeria.arrangeProductsBySimilarity(controladorPrestatgeria.getProductesColocats(), controladorPrestatgeria.getAltura(),mantenerCambios);
        controladorPrestatgeria.getProductesColocats().clear();
        controladorPrestatgeria.getProductesColocats().addAll(arrangedProductes);
        int medida;
        if (controladorPrestatgeria.getProductesColocats().size()% controladorPrestatgeria.getAltura() == 0) {
            medida = controladorPrestatgeria.getProductesColocats().size()/controladorPrestatgeria.getAltura();
        } else {
            medida = controladorPrestatgeria.getProductesColocats().size()/controladorPrestatgeria.getAltura() + 1;
        }

        String[][] matrix = new String[controladorPrestatgeria.getAltura()][medida];
        for (ProducteColocat producteColocat : controladorPrestatgeria.getProductesColocats()) {
        ...
        for (int j = 0; j < medida; j++) {
            for (int i = 0; i < controladorPrestatgeria.getAltura(); i++) {
                if (matrix[i][j] != null) {
    ...
    private static void consultaDistribucio_Ordenada_Actual() {
        ...
        if (respuesta.equalsIgnoreCase("s")) {
            ...
            for (int i = 0; i < controladorPrestatgeria.getProductesColocats().size(); i++) {
                System.out.println((i + 1) + ". " + controladorPrestatgeria.getProductesColocats().get(i).getProducte().getNom());
            }
            ...
            controladorPrestatgeria.intercambiar_productes(seleccion1 - 1, seleccion2 - 1);
        }
    }
    ...
    private static void consultaProductes() {
        ArrayList<String> productes = controladorPrestatgeria.getProductes();
        int n = productes.size();
        if (n == 0) {
            error_no_hi_ha_productes();
            return;
        }
        System.out.println("Els productes de la prestatgeria son:");
        for (int i = 0; i < n; ++i) {
    ...
    private static void agregarProducte() {
        ...
        if (controladorPrestatgeria.existeix_producte(nombre)) {
       ...
       int pos = (controladorPrestatgeria.getProductesColocats().size() + 1);
       int altura = (Math.min(cantidad, controladorPrestatgeria.getAltura()));
       controladorPrestatgeria.afegeix_producte(nombre, marca, precio, cantidad, pos, altura);
    ...
    private static void eliminarProducte() {
        ...
        if (!controladorPrestatgeria.existeix_producte(producte)) {
            System.out.println("ERROR - Aquest producte no existeix ");
            return;
        }
        controladorPrestatgeria.eliminaProducte(producte);
    ...
    private static void modificarProducte() {
        ...
        if (respuesta.equals("S") || respuesta.equals("s")) {
            ...
            scanner.nextLine(); // Consumir nueva línea
            controladorPrestatgeria.set_cantidad_producto(producte,nuevaCantidad);
            controladorPrestatgeria.set_precio_producto(producte,nuevoPrecio);
    ...
    private static void modificaSimilitudProducte(String Producto) {
        while (true) {
            ...
            controladorPrestatgeria.set_similitud(Producto, similituds_del_producte.get(seleccion - 1), valorSimilitud);
    ...
    private static ArrayList<String> imprimeSimilitud(String producto) {
        ...
        ArrayList<String> resultat = controladorPrestatgeria.getSimilitudsProducte(producto);
        if (resultat.isEmpty()) {
    ...
    private static void consulta_info_producte(String producte) {
        ArrayList<String> info = controladorPrestatgeria.get_info_producte(producte);
