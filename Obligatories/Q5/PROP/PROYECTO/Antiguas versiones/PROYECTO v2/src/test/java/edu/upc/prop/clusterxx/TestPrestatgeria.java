...
public class TestPrestatgeria {
    ...
    @Test
    public void testGenerarPermutaciones() throws Exception {
        ProducteColocat[] productos = {producteColocat};
        List<ProducteColocat[]> permutaciones = new ArrayList<>();
        Method method = Prestatgeria.class.getDeclaredMethod("generarPermutaciones", ProducteColocat[].class, int.class, int.class, List.class);
        method.setAccessible(true);
        method.invoke(prestatgeria, productos, 0, productos.length, permutaciones);
        assertEquals(1, permutaciones.size());
        assertEquals(producteColocat, permutaciones.get(0)[0]);
    }
