...
public class EstrategiaTSP implements EstrategiaCalculo {
    ...
    public List<ProducteColocat> arrangeProductsBySimilarity(List<ProducteColocat> productes, int altura, boolean mantenerCambios) {
        ...
        // Aplicamos Prim para obtener el MST
        ArrayList<Edge> mst = prim(aristas, productes);

        // Construimos el grafo dirigido T' reemplazando cada arista de T por dos arcos de direcciones opuestas
        ...
        // Preparamos la salida, pasando de string a productes colocats
        return prepara_salida(productes, ciclo_hamiltoniano);
    }
    ...
    private ArrayList<Edge> generateEdges(List<ProducteColocat> productes) {
        ...
        for (int i = 0; i < n; i++) {
            for (int j = i + 1; j < n; j++) {
                ...
                edges.add(new Edge(productes.get(i).getProducte().getNom(), productes.get(j).getProducte().getNom(), similitud));
            }
        }
        return edges;
    }
    ...
    private ArrayList<String> cicloHamiltoniano(ArrayList<Edge> cicloEuleriano, ArrayList<Edge> el_resto_de_aristas, ArrayList<ProducteColocat> productes_totals) {
        // Lo ordenamos por si no viene ordenado
        ...
        // Recorremos cada nodo y hacemos un ciclo hamiltoniano de cada nodo con las aristas del ciclo euleriano
    ...
    private int calcula_similitud(ArrayList<String> posibleSolucion, ArrayList<Edge> aristas) {
        ...
        resultado += get_similitud_2_productes(posibleSolucion.get(0), posibleSolucion.get(posibleSolucion_size - 1), aristas);
        return resultado;
    }
