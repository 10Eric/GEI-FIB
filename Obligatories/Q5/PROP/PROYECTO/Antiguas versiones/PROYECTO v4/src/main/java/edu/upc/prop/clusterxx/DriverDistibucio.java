...
public class DriverDistibucio {
    ...
    private static void mostrarOpcionesDistribucio() {
        while (true) {
            ...
            switch (opcion) {
                ...
                case 7 -> consultaDistribucio_Ordenada_Actual();  //imprime la Distribucio entera
    ...
    private static void calcular_distribucion() {
        ...
        List<ProducteColocat> arrangedProductes = controladorDistribucio.arrangeProductsBySimilarity(controladorDistribucio.getProductesColocats(), altura,mantenerCambios);
        controladorDistribucio.getProductesColocats().clear();
        controladorDistribucio.getProductesColocats().addAll(arrangedProductes);
        int medida;
        if (controladorDistribucio.getProductesColocats().size()% altura == 0) {
            medida = controladorDistribucio.getProductesColocats().size()/altura;
        } else {
            medida = controladorDistribucio.getProductesColocats().size()/altura + 1;
        }

        String[][] matrix = new String[altura][medida];
        for (ProducteColocat producteColocat : controladorDistribucio.getProductesColocats()) {
