...
#ifndef ROWS
#define ROWS 32                 /* size of display window in pixels */
#endif

#ifndef COLS
#define COLS 32                 /* size of display window in pixels */
#endif
...
{
      int equal, vertical_equal, horizontal_equal;
      ...
      for (int y = 0; y < ROWS; y += TILE) {
	    for (int x = 0; x < COLS; x += TILE) {
		  ...
		  tareador_start_task("vertical_borders");
		  vertical_equal = 1;
		  for (int py = y; py < y + TILE; py++) {
			...
			vertical_equal = vertical_equal & (M[y][x] == M[py][x]);
			vertical_equal = vertical_equal & (M[y][x] == M[py][x + TILE - 1]);
		  }
		  tareador_end_task("vertical_borders");
		  
		  //check horizontal borders
		  tareador_start_task("horizontal_borders");
		  horizontal_equal = 1;
		  for (int px = x; px < x + TILE; px++) {
			...
			horizontal_equal = horizontal_equal & (M[y][x] == M[y][px]);
			horizontal_equal = horizontal_equal & (M[y][x] == M[y + TILE - 1][px]);
		  }
		  tareador_end_task("horizontal_borders");

		  //check if we can avoid computation of tile
		  tareador_start_task("if-else");
		  equal = vertical_equal & horizontal_equal;
		  if (equal) {
			if (output2histogram)
			...
			for (int py = y; py < y + TILE; py++) {
			      tareador_disable_object(&M[y][x]);
			      tareador_start_task("equal-py");
			      for (int px = x; px < x + TILE; px++) {
				    M[py][px] = M[y][x];
				    if (output2display) {
			      ...
			      tareador_end_task("equal-py");
			      tareador_enable_object(&M[y][x]);
            }
		  } else {
			// computation of tile
			for (int py = y; py < y + TILE; py++) {
			      tareador_start_task("computation py");
			      for (int px = x; px < x + TILE; px++) {
			      ...
			      tareador_end_task("computation py");
            		}
		  }
		tareador_end_task("if-else");
...
int main(int argc, char *argv[])
{
      ...
      int height = ROWS;
      double size = N, x0 = -1.81641, y0= -0.203125;
      int *Hmatrix;
      ...
      for (int i = 1; i < argc; i++) {
	    if (strcmp(argv[i], "-d") == 0) {
		  ...
		  fprintf(stderr, "       -o to write the computed histogram (mandel_histogram.out) to disk (default no file generated)\n");
