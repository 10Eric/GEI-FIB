...
void mandel_tiled_rec(int M[ROWS][COLS], int NRows, int NCols, int start_fil, int start_col, double CminR, double CminI, double CmaxR, double CmaxI, double scale_real, double scale_imag, int maxiter, int depth)
{
      int non_first_call= (NRows!=ROWS) | (NCols!=COLS);
      int equal,equalX,equalY;
      int x, y;

      equal = non_first_call;
      equalX = non_first_call;
      equalY = non_first_call;
      y = start_fil;
      x = start_col;
      for (int py = y; py < y + NRows; py++) {
	    ...
	    equalY = equalY & (M[y][x] == M[py][x]);
	    equalY = equalY & (M[y][x] == M[py][x + NCols - 1]);
      }
      ...
      for (int px = x; px < x + NCols; px++) {
	    ...
	    equalX = equalX & (M[y][x] == M[y][px]);
	    equalX = equalX & (M[y][x] == M[y + NRows - 1][px]);
      }
      equal = equalX & equalY;
		...
		#pragma omp task firstprivate(y,x)
		  for (int py = y; py < y + NRows; py++)
	   	 ...
	   	 if (depth < CUTOFF){
			  if (NRows > TILE) {
				#pragma omp task 
				mandel_tiled_rec(M, NRows / 2, NCols / 2, start_fil, start_col, CminR, CminI, CmaxR, CmaxI, scale_real, scale_imag, maxiter, depth+1);
				#pragma omp task 
				mandel_tiled_rec(M, NRows / 2, NCols / 2, start_fil, start_col + NCols / 2, CminR, CminI, CmaxR, CmaxI, scale_real, scale_imag, maxiter, depth+1);
				#pragma omp task 
				mandel_tiled_rec(M, NRows / 2, NCols / 2, start_fil + NRows / 2, start_col, CminR, CminI, CmaxR, CmaxI, scale_real, scale_imag, maxiter, depth+1);
				#pragma omp task 
				mandel_tiled_rec(M, NRows / 2, NCols / 2, start_fil + NRows / 2, start_col + NCols / 2, CminR, CminI, CmaxR, CmaxI, scale_real, scale_imag, maxiter, depth+1);
			  } else {
				#pragma omp task
				mandel_tiled_rec(M, NRows, NCols / 2, start_fil, start_col, CminR, CminI, CmaxR, CmaxI, scale_real, scale_imag, maxiter, depth+1);
				#pragma omp task 
				mandel_tiled_rec(M, NRows, NCols / 2, start_fil, start_col + NCols / 2, CminR, CminI, CmaxR, CmaxI, scale_real, scale_imag, maxiter, depth+1);
			  }
			  #pragma omp taskwait
		}
		    else {
...
int main(int argc, char *argv[])
{
      ...
      int height = ROWS;
      double size = N, x0 = -1.82641, y0= -0.204125;
      int *Hmatrix;
      unsigned char *image;
      TRACE_START_MARK;
