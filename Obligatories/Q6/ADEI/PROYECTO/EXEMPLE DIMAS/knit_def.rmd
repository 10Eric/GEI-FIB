...
columnas_a_convertir <- c( "country",  "dealer")
...
columnas_a_eliminar <- c("url", "reference", "location", "dealer_url", 
                         "dealer_is_professional", "description", "title")

# Eliminar las columnas del dataset
dd <- dd[, !(names(dd) %in% columnas_a_eliminar)]
```
...
dcat<-c(1,3,5,6) #Escoger variables categoricas
...
```{r}
# ATTACH THE DATAFRAME AND PREPARE NUMERICAL VARIABLES
attach(dd)
names(dd)

# Select relevant numerical variables for clustering
dcon <- data.frame(rooms, baths, area, latitude, longitude, price)
dim(dcon)
...
Tss <- k1$totss                             # Total sum of squares
Ib1 <- 100 * Bss / (Bss + Wss)              # Explained inertia percentage
print(paste("Explained Inertia (%):", Ib1))

# Visualize cluster centers
plot(k1$centers[, 2], k1$centers[, 3], main="Cluster Centers", xlab="baths", ylab="area", col="blue")
points(k1$centers, col=1:5, pch=19)

# Repeat K-Means for K=8
k2 <- kmeans(dcon, centers = 8)
Ib2 <- 100 * sum(rowSums(k2$centers^2) * k2$size) / (sum(k2$withinss) + sum(rowSums(k2$centers^2) * k2$size))
print(paste("Explained Inertia (K=8):", Ib2))
...
d <- dist(dcon)  # Euclidean distance
h1 <- hclust(d, method = "ward.D")  # Ward's method

# Plot dendrogram
plot(h1, main = "Hierarchical Clustering Dendrogram", sub = "", xlab = "")
...
# Visualize Clusters
pairs(dcon[, 1:4], col = c1, main = "Hierarchical Clustering Visualization")

# Compute group means (centroids)
cdg <- aggregate(dcon, list(c1), mean)
print("Cluster Centroids:")
print(cdg)

# Scatter plots for selected variables
plot(price, area, col = c1, main = "Price vs Area Clustering", xlab = "Price", ylab = "Area")
legend("topright", legend = paste("Class", 1:nc), col = 1:nc, pch = 19)
...
# Compute Gower's distance for mixed numerical/categorical data
actives <- c("rooms", "baths", "area", "latitude", "longitude", "price", "operation", "property_type")
dissimMatrix <- daisy(dd[, actives], metric = "gower", stand = TRUE)
distMatrix <- dissimMatrix^2

# Perform hierarchical clustering
h2 <- hclust(distMatrix, method = "ward.D")
plot(h2, main = "Hierarchical Clustering with Gower's Distance", sub = "", xlab = "")
...
boxplot(dd$price ~ c2, horizontal = TRUE, main = "Price Distribution by Cluster", col = "lightblue")

# Visualize Clusters with Selected Variables
plot(dd$rooms, dd$price, col = c2, main = "Rooms vs Price Clustering", xlab = "Rooms", ylab = "Price")
