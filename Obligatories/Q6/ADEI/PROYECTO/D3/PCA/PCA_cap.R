...
dd<-read.csv("~/Documents/ADEI/Project/adult_def.csv", stringsAsFactors = TRUE)
set.seed(123)
...
objects()
attributes(dd)
dd <- dd[, !(names(dd) %in% c("fnlwgt", "income_integer", "edu_num"))]
...
legend("bottomright",levels(factor(varcat)),pch=1,col=varcat, cex=0.6)

#select your qualitative variable
k<-16 #sex (male or female)
#names(dd)
varcat<-factor(dd[,k])
...
dd$cap_gain_loss_tocada <- dd$cap_gain_tocada && dd$cap_loss_tocada

for(i in 1:nrow(dd)) {
  if(dd[i, 9] == 0 & dd[i, 10] == 0){
    dd[i,16] <- 0
  }
  else
    dd[i,16] <- 1
