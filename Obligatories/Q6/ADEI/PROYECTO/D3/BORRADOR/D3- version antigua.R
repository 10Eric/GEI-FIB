...
names(dd)
boxplot(age ~ cluster, data=dd, horizontal=TRUE, main="Edat per Clúster")
boxplot(hours_week ~ cluster, data=dd, horizontal=TRUE, main="Hores setmanals per Clúster")
boxplot(cap_gain ~ cluster, data=dd, horizontal=TRUE, main="Guany capital per Clúster")
pairs(dd[, c("age", "hours_week", "cap_gain")], col=dd$cluster)
cdg <- aggregate(dd[, c("age", "edu_num", "cap_gain", "cap_loss", "hours_week")],
                 by = list(cluster=dd$cluster), FUN = mean)
cdg

barplot(table(dd$workclass, dd$cluster), beside=TRUE, col=rainbow(length(levels(dd$workclass))))
legend("topright", levels(dd$workclass), fill=rainbow(length(levels(dd$workclass))), cex=0.5)
